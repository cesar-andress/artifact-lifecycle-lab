...
1. ``[[REF]]` copies `src/prefect/` into a temp directory, **deletes** server-only and CLI code, then builds with `client/pyproject.toml`. The resulting package has the same version as `prefect` but fewer dependencies.`

2. `- **Manual** — `bash client/[[REF]]``

3. `If the CI build fails, reproduce locally with `bash client/[[REF]]` and run the smoke tests (`client_flow.py`, `client_deploy.py`).`
