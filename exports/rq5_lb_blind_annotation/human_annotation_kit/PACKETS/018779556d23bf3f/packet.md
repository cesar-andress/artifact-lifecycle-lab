...
Referenced artifact R1 denotes a file path cited by the project instruction text. Its identity is withheld as a path string so treatment assignment cannot be inferred. The instruction context around the citation is provided below with the path replaced by [[REF]].
...
1. `This app takes `[[REF]]` and bundles it into 6 separate webpack entry points deployed to `widgets.wp.com/help-center/`. Jetpack enqueues these bundles on various types of websites and pages (editor, wp-admin, logged out pages (/support and /forums), CIAB)`

2. `Each entry point is a standalone JS file in the app root (e.g., `help-center-gutenberg.js`) that imports from `[[REF]]` and wires up the environment-specific bootstrap logic.`
